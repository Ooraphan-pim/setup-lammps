...
DumpData::DumpData(int narg, char **arg) : Dump(narg, arg)
{
  if (narg != 5) error->all("Illegal dump data command");
...
void DumpData::init()
{
  ...
  if (format_user) {
    ...
    char *str;
    if (image_flag == 0) str = "%d %d %25.15g %25.15g %25.15g";
    else str = "%d %d %25.15g %25.15g %25.15g %d %d %d";
    int n = strlen(str) + 2;
    format = new char[n];
    strcpy(format,str);
    strcat(format,"\n");
  ...
  if (binary) {
    header_choice = &DumpData::header_binary;
  } else {
    if( image_flag == 1 ) {
      header_choice = &DumpData::header_item;
    } else {
      header_choice = &DumpData::header_dlo;
    }
  }

  if (scale_flag == 1 && image_flag == 0)
    pack_choice = &DumpData::pack_scale_noimage;
  else if (scale_flag == 1 && image_flag == 1)
    pack_choice = &DumpData::pack_scale_image;
  else if (scale_flag == 0 && image_flag == 0)
    pack_choice = &DumpData::pack_noscale_noimage;
  else if (scale_flag == 0 && image_flag == 1)
    pack_choice = &DumpData::pack_noscale_image;

  if (binary) write_choice = &DumpData::write_binary;
  else if (image_flag == 0) write_choice = &DumpData::write_noimage;
  else if (image_flag == 1) write_choice = &DumpData::write_image;
}

/* ---------------------------------------------------------------------- */

int DumpData::modify_param(int narg, char **arg)
{
...
void DumpData::write_header(int ndump)
{
...
void DumpData::write_vel_header(int ndump)
{
  fprintf(fp,"\n");
...
void DumpData::write_data(int n, double *buf)
{
...
void DumpData::header_binary(int ndump)
{
...
void DumpData::header_dlo(int ndump)
{
  //  fprintf(fp,"ITEM: TIMESTEP\n");
  fprintf(fp,"%d  timestep  dump_data\n",update->ntimestep);
  //  fprintf(fp,"dump_data\n");
  fprintf(fp,"\n");
...
void DumpData::header_item(int ndump)
{
  fprintf(fp,"ITEM: TIMESTEP\n");
  fprintf(fp,"%d\n",update->ntimestep);
  fprintf(fp,"ITEM: NUMBER OF ATOMS\n");
...
int DumpData::pack_scale_noimage()
{
  int *tag = atom->tag;
  int *type = atom->type;
  int *mask = atom->mask;
...
int DumpData::pack_noscale_image()
{
  int *tag = atom->tag;
  int *type = atom->type;
  int *image = atom->image;
...
int DumpData::pack_noscale_noimage()
{
  int *tag = atom->tag;
  int *type = atom->type;
  int *mask = atom->mask;
...
void DumpData::write_binary(int n, double *buf)
{
  n *= size_one;
...
void DumpData::write_image(int n, double *buf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp,format,
...
void DumpData::write_noimage(int n, double *buf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp,format,
...
void DumpData::write_vel(int n, double *buf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp,"%d %25.15g %25.15g %25.15g\n",
	    static_cast<int> (buf[m]),
	    buf[m+5],buf[m+6],buf[m+7]);
    m += size_one;
  }
}
