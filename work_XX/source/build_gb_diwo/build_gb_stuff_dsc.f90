  ...
  contains
    subroutine do_atom()
      ...
      if( island_flag ) then
         ...
         if( double_edge ) then
            if( x_dir * trans_pos(1) .ge. x_high_cutoff - 2.0d0 * edge_size ) then
               crystal_ii = crystal_ii + 3    !! It is an inner edge atom, to be "fixed"
            end if
            if( x_dir * trans_pos(1) .ge. x_high_cutoff - edge_size ) then
               crystal_ii = crystal_ii + 2    !! It is an outer edge atom, to be "fixed"
            end if
         else
            if( x_dir * trans_pos(1) .ge. x_high_cutoff - edge_size ) then
         ...
         if( jiggle_atoms ) then
            if( x_dir * trans_pos(1) .le. jiggle_region ) then
               !!  This is an atom near the boundary and we are to jiggle it
               call jiggle_position( trans_pos, jiggle_amount )
            end if
         end if
