...
#else
extern "C"  int  diwo_continue_build_not_e_or_p__();
#endif
...
//   }
  ...
  while(1) {
    ...
    //  FOR NEW LAMMPS
    //     Here we can get the energy and pass it to diwo_continue_build_not_e_or_p


//   In control file
//     compute  free_pe_atom  free_atoms  pe/atom
//     compute  free_pe       free_atoms  reduce  sum free_pe_atom

//   In code
//     int icompute = modify->find_compute("free_pe");
//     if (icompute < 0) 
//       error->all("compute ID free_pe not found");
//     double  free_pe = modify->compute[icompute];
...
#else
      finished = diwo_continue_build_not_e_or_p__();
#endif
