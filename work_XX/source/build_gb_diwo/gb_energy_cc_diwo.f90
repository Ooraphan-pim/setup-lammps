...
program gb_energy_cc_diwo
  ...
  use delete_file_mod
  use system_mod
  use build_gb_stuff_dsc_mod, only: xi_chi_files
  implicit none
...
!!  character(256) :: name_in
  ...
  if( .not. build_xi_chi_files ) then
     if( do_now_flag ) then
...
!! if build_xi_chi_files is true, just do that (they depend on temperature, but build does not).

        if( build_xi_chi_files ) then
           call check_build_gb_control(init)
...
!!             call for_flush(6)
             ...
             if( jiggle_atoms ) then
                init%jiggle_seed = 1 + num_init + jiggle_seed
             end if
